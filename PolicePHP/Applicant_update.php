<?php
    ...
    $sql = "CALL searchApplicant('".$applicant_id."')";
    $ret = mysqli_query($con, $sql);
    if($ret) {
            $row = mysqli_fetch_array($ret);
    }
    else {
        echo "신고자 데이터 조회 실패 !!!!"."<br>";
...
<h3> &nbsp&nbsp&nbsp&nbsp&nbsp신고자 정보 수정 </h3>
...
<form method = "post">
    ...
    ?>
	신고일시: <input type = "datetime-local" name = "applicant_date" max = "2022-11-28" value = "<?php echo $date ?>" required> <br>
    <hr>
    <?php
    echo "<input type = 'text' name = 'role' value ='".$role."' hidden>";
    echo "<input type = 'text' name = 'teamId' value ='".$teamId."' hidden>";
    ?>
    <button type = "submit" formaction = "/PolicePHP/Applicant_update_result.php?applicant_id=<?php echo $applicant_id; ?>>"> 수정완료</button>
</form>
</body>
</html>
