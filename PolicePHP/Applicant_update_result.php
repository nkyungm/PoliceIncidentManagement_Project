<!DOCTYPE html>
    <html>
    <head>
    <meta charest="utf-8">
    </head>

    <body>
    <hr width = "100%" color = "#1369B8" size = "60">
    <h3> &nbsp&nbsp&nbsp&nbsp&nbsp신고자 정보 등록 결과</h3>
    <hr width = "100%" color = "#1369B8" size = "10">
    <br>

<?php
    $con = mysqli_connect("policedb.cg9boi0b9viu.ap-northeast-2.rds.amazonaws.com", "admin", "rnqhstlr123!", "PoliceDB") 
    or die("MySQL 접속 실패!!");

    $applicant_id = $_GET['applicant_id'];
    $role = $_POST["role"];
    $teamId = $_POST["teamId"];
    $name = $_POST["name"];
    $birth = $_POST["birth"];
    $phone = $_POST["phone"];
    $date = $_POST["applicant_date"];
    $gender = $_POST["gender"];
    $address = $_POST["address"];

    $sql = "CALL updateApplicant('".$applicant_id."', '".$name."', '".$birth."', '".$phone."', '".$date."', '".$gender."', '".$address."')";
    $ret = mysqli_query($con, $sql);

    if($ret) {
        echo "신고자 정보 정상 수정 완료.";
    }
    else {
        echo "신고자 정보 수정 실패 !!!";
        echo "실패 원인 :" .mysqli_error($con);
        echo "<hr> <a href = '/PolicePHP/home.php?role=$role&teamId=$teamId'><button>확인</button></a> ";
    }
    mysqli_close($con);

    echo "<hr> <a href = '/PolicePHP/home.php?role=$role&teamId=$teamId'><button>확인</button></a> ";
    
?>
    
    </body>

    </html>

